...
}).with('password', 'confirmPassword').messages({ 'any.only': 'Passwords do not match.' })
...
const requestResetPassSchema = Joi.object({
    email: Joi.string().email({ minDomainSegments: 2 }).required()
})

const resetPassSchema = Joi.object({
    token: Joi.string().required('Token invalid.'),
    password: Joi.string().pattern(/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[!@#$%^&*()\-=_+{}[\]|;:",.<>/?]).{6,20}$/)
        .required()
        .messages({
            'string.pattern.base': "Field Password must contain betwwen 8 to 20 characters (minimal 1 uppercase, 1 special and 1 numerical character) "
...
}).with('password', 'confirm_password').messages({ 'any.only': 'Passwords do not match.' })
